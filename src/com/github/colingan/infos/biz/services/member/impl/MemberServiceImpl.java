 ...
 */
@Service
public class MemberServiceImpl extends AbstractBaseService implements
		MemberService {
	...
	public Member queryMemberInfoForUser(String userName) {
		if (StringUtils.isEmpty(userName)) {
			throw new IllegalArgumentException(
					"userName is required while query member infos. - "
							+ userName);
		}
		List<Member> members = innerQueryMembers(null, userName, Boolean.TRUE,
				null);
	...
	private List<Member> innerQueryMembers(Long id, String userName,
			Boolean validate, List<Field> fields) {
		Condition condition = null;
		Condition firstCondition = null;
		if (id != null) {
			firstCondition = new ComparisonCondition(Field.ID, Operation.EQ,
					id.longValue());
		}
		condition = compoundCondition(condition, firstCondition,
				LogicOperation.AND);
		if (!StringUtils.isEmpty(userName)) {
			firstCondition = new ComparisonCondition(Field.USER_NAME,
					Operation.EQ, userName);
		}
		condition = compoundCondition(condition, firstCondition,
				LogicOperation.AND);
		if (validate != null) {
		...
		return memberDao.getObjects(null, new ArrayList<Field>(fieldSet),
				condition, orderBy);
	}

	@Override
	public Member addMember(String userName, int role) {
		...
		if (!member.validate()) {
			throw new RuntimeException("member info is not complete - "
					+ member);
		}
	...
	public boolean delete(long id) {
		return this.memberDao.delete(null, new ComparisonCondition(Field.ID,
				Operation.EQ, id)) == 1;
	}
