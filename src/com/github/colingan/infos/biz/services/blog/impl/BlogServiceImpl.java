...
public class BlogServiceImpl implements BlogService {
	...
	private static final String BLOG_FILE_PREFIX = "_ONES_BLOGS_";

	@ConfContextWatch(key = "blog.dir", confCenter = Constants.DEFUALT_CONCENTER_FILE, file = Constants.SYSCONF_FILE)
	...
	public boolean deleteBlog(long id) {
		return this.blogDao.delete(null, new ComparisonCondition(Field.ID,
				Operation.EQ, id)) == 1;
	}
	...
	public Blog queryBlogBrief(long id) {
		...
		Condition condition = new ComparisonCondition(Field.ID, Operation.EQ,
				id);
	...
	@Override
	public List<Blog> queryBlogsBrief(long rootCategory, long childCategory,
		...
		Condition condition = new LogicGroupCondition(new ComparisonCondition(
				Field.CATEGORY2, Operation.EQ, childCategory),
				LogicOperation.AND, new ComparisonCondition(Field.CATEGORY1,
						Operation.EQ, rootCategory));
		OrderBy orderBy = new CommonOrderBy(Field.ID, OrderByDirection.DESC);

		long offset = (pageNum - 1) * pageSize;
		offset = offset < 0 ? 0 : offset;
		return this.blogDao.getObjects(null, fields, condition, orderBy,
				offset, pageSize);
	}
	...
	public Blog queryBlogDetail(long id) {
		...
		try {
			blog.setContent(IOUtils.toString(new FileInputStream(file),
					Constants.UTF_8));
		} catch (Exception e) {
	...
	@Override
	public long addNewBlog(String userName, String title, long category1,
