...
@Controller
public class BlogController extends BaseController implements
	...
	public String blogsRootEntry(HttpServletRequest request,
		...
		List<Category> childCategorys = this.categoryService
				.getValidSecondChildLevelContent(id);
		...
		return "redirect:/blogList?rootCategory=" + id + "&childCategory="
				+ firstChild.getId();
	}

	@RequestMapping(value = "/blogList")
	public String listBlogs(HttpServletRequest request,
			HttpServletResponse response) {
		Long rootCategory = null;
		try {
			rootCategory = Long.parseLong(request
					.getParameter(Field.ROOT_CATEGORY.getKeyName()));
		...
		try {
			childCategory = Long.parseLong(request
					.getParameter(Field.CHILD_CATEGORY.getKeyName()));
			Validate.notNull(childCategory);
		} catch (Exception e) {
			throw new IllegalArgumentException("child category缺失 or 不正确！");
		}
		List<Category> children = this.categoryService
				.getValidSecondChildLevelContent(rootCategory);
		if (CollectionUtils.isEmpty(children)
				|| !children.contains(new Category(childCategory))) {
		...
		List<Blog> blogList = this.blogService.queryBlogsBrief(rootCategory,
				childCategory, pageNum, pageSize);
		...
		long totalCount = this.blogService.getBlogSize(rootCategory,
				childCategory);
		model.setTotalPage(totalCount % pageSize == 0 ? totalCount / pageSize
	...
	public String newBlogEntry(HttpServletRequest request,
			HttpServletResponse response) {
	...
	public String editBlogEntry(HttpServletRequest request,
			HttpServletResponse response) {
	...
	public String deleteBlog(HttpServletRequest request,
		...
		try {
			this.blogOwnerCheck(request, super.getUserName(request),
					blog.getUserName());
		} catch (Exception e) {
			throw new PermissionDeniedException("您没有该文章的编辑权限！");
		}
		if (this.blogService.deleteBlog(id)) {
			return "redirect:/blogList?rootCategory=" + blog.getCategory1()
	...
	public Map<String, Object> updateBlog(HttpServletRequest request,
		...
		try {
			blogId = Long
					.parseLong(request.getParameter(Field.ID.getKeyName()));
	...
	@ResponseBody
	public Map<String, Object> submitNewBlog(HttpServletRequest request,
			HttpServletResponse response) {
		JTableModel model = new JTableModel();
		String title = request.getParameter(Field.TITLE.getKeyName());
		if (StringUtils.isEmpty(title)) {
		...
		Long category1 = null;
		try {
			category1 = Long.parseLong(request.getParameter(Field.CATEGORY1
					.getKeyName()));
		...
		Long category2 = null;
		try {
			category2 = Long.parseLong(request.getParameter(Field.CATEGORY2
					.getKeyName()));
		...
		String needBroadcast = request.getParameter(Field.NEED_BROADCAST
				.getKeyName());
		Set<String> mailSet = new HashSet<String>();
		if (BizConstants.CHECKED.equals(needBroadcast)) {
			String users = request.getParameter(Field.USERS.getKeyName());
			if (StringUtils.isNotEmpty(users)) {
				// split by comma
				String[] mails = users.split(Constants.COMMA);
				for (String mail : mails) {
					if (!this.validateEmail(mail)) {
						model.fail("邮箱不合法 - " + mail);
						return model.getDatas();
					}
					mailSet.add(mail);
				}
		...
		try {
			...
			long blogId = this.blogService.addNewBlog(userName, title,
					category1, category2, content);
			if (blogId == 0) {
				model.fail("文件存储失败！");
			} else {
				model.success().setRedirectPage("/blogs/view?id=" + blogId);
				// publish broadcast event if needs
				if (CollectionUtils.isNotEmpty(mailSet)) {
					// TODO no need to mail author
				}
			}
		} catch (Exception e) {
			model.fail("系统异常！");
	...
	@ResponseBody
	public BlogModel viewBlogRaw(HttpServletRequest request,
			HttpServletResponse response) {
	...
	protected void basePublishMemberCheck(HttpServletRequest request) {
		...
		Validate.isTrue(
				member.getRoleGroup() >= RoleGroup.READ_WRITE.getValue(),
				"member role should greater than administrator");
	}

	protected void blogOwnerCheck(BlogModel model, HttpServletRequest request) {
		this.blogOwnerCheck(request, model.getBasic().getUserName(),
				model.getAuthor());
	}

	private void blogOwnerCheck(HttpServletRequest request, String userName,
			String blogAuthor) {
	...
	private boolean validateCategory(long level1, long level2) {
		try {
			Category category = this.categoryService.getValidateCategoryByIds(
					level1, level2);
	...
	private BlogModel innerQueryBlogModel(HttpServletRequest request) {
		...
		// breadcrumb
		List<Category> categorys = this.categoryService.queryCategorys(
				Long.valueOf(blog.getCategory1()),
			...
			breadcrumb.add(new HashMap<String, Object>() {
				{
					...
					if (category.getLevel() == 1) {
						put(Field.LINK.getKeyName(),
								"/blogs/root?id=" + blog.getCategory1());
					} else {
						put(Field.LINK.getKeyName(), "/blogList?rootCategory="
								+ blog.getCategory1() + "&childCategory="
								+ blog.getCategory2());
					}
				}
			});
