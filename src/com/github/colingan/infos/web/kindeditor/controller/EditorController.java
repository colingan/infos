...
public class EditorController extends BaseController {
    ...
    @ConfContextWatch(key = "nav.colors", confCenter = DEFUALT_CONCENTER_FILE, file = SYSCONF_FILE)
    protected volatile String colors;
        ...
        if (currentPathFile.listFiles() != null) {
            for (File file : currentPathFile.listFiles()) {
                ...
                if (file.isDirectory()) {
                    ...
                    hash.put("is_photo", VALID_FILE_EXT.get("image").contains(fileExt));
    ...
    public Map<String, Object> upload(MultipartHttpServletRequest request, HttpServletResponse response) {
        ...
        String saveUrl = BizConstants.EDITOR_SAVE_URL + dirName + "/";
        ...
        for (Entry<String, MultipartFile> entry : fileMap.entrySet()) {
            ...
            if (fileSize > MAX_FILE_SIZE) {
                return getError("file size should less than " + MAX_FILE_SIZE + ", current is : " + fileSize);
            }
            String ext = FileUtil.getExtension(fileName);
            if (!VALID_FILE_EXT.get(dirName).contains(ext.toLowerCase())) {
                return getError("file extension not allowed." + ext);
            }
            String dest = completeFormat.format(new Date()) + "_" + random.nextInt(1000) + "." + ext.toLowerCase();
            try {
