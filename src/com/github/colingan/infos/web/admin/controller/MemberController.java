...
public class MemberController extends AdminController {
	...
	public Map<String, Object> listMembers(HttpServletRequest request,
		...
		try {
			model.addAllRecord(this.memberService.queryAllValidateMembers())
					.success();
		} catch (Exception e) {
	...
	@ResponseBody
	public Map<String, Object> updateMember(HttpServletRequest request,
			HttpServletResponse response) {
		super.baseAdminMemberCheck(request);
		JTableModel model = new JTableModel();
		String idStr = request.getParameter(Field.ID.getKeyName());
	...
	public Map<String, Object> addMember(HttpServletRequest request,
		...
		Member member = null;
		try {
			member = this.memberService.addMember(userName, role);
