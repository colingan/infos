...
public class SliderController extends AdminController {
	...
	public Map<String, Object> uploadImage(MultipartHttpServletRequest request,
		...
		return super.imageUploaduploadImages(request,
				this.sliderService.getImageFileDirectory(), true);
	}

	@RequestMapping(value = "/admin/slider")
	public String slider(HttpServletRequest request,
			HttpServletResponse response) {
	...
	@ResponseBody
	public Map<String, Object> listSliders(HttpServletRequest request,
			HttpServletResponse response) {
		super.baseAdminMemberCheck(request);
		JTableModel model = new JTableModel();
		try {
			model.addAllRecord(this.sliderService.queryAllValidateSliders())
					.success();
		} catch (Exception e) {
	...
	@ResponseBody
	public Map<String, Object> deleteSlider(HttpServletRequest request,
			HttpServletResponse response) {
		super.baseAdminMemberCheck(request);
		JTableModel model = new JTableModel();
	...
	public Map<String, Object> addOrUpdateSlider(HttpServletRequest request,
		...
		try {
			idx = Integer
					.parseInt(request.getParameter(Field.IDX.getKeyName()));
			Validate.notNull(idx);
		} catch (Exception e) {
			model.fail("序号不能为空！");
			return model.getDatas();
		}
		String originName = request
				.getParameter(Field.ORIGIN_NAME.getKeyName());
		...
		if (id == null) {
			...
			try {
				model.setRecord(
						this.sliderService.addSlider(idx, originName, destName))
						.success();
			} catch (Exception e) {
				model.fail("系统异常！");
			}
			...
			// update
			try {
				if (this.sliderService.updateSlider(id, idx, originName,
						destName)) {
					model.success();
				} else {
					model.fail("更新失败！");
