...
public class CategoryController extends AdminController {
	...
	public Map<String, Object> uploadImage(MultipartHttpServletRequest request,
		...
		return super.imageUploaduploadImages(request,
				this.categoryService.getIconFileDirectory(), false);
	}
	...
	public Map<String, Object> queryChildCategory(HttpServletRequest request,
		...
		String parentCategory = request.getParameter(Field.PARENT_CATEGORY
				.getKeyName());
		...
		try {
			List<Category> datas = this.categoryService
					.getValidSecondChildLevelContent(pid);
	...
	public String categoryConsole(HttpServletRequest request,
			HttpServletResponse response) {
		this.baseAdminMemberCheck(request);
	...
	public Map<String, Object> addCategory(HttpServletRequest request,
		...
		String icon = request.getParameter(Field.ICON.getKeyName());
		...
		Integer level = null;
		try {
			level = Integer.parseInt(request.getParameter(Field.LEVEL
					.getKeyName()));
		...
		try {
			parentCategory = Long.parseLong(request
					.getParameter(Field.PARENT_CATEGORY.getKeyName()));
		...
		try {
			Category category = this.categoryService.addCategory(
					super.getUserName(request), name, icon, level,
					parentCategory);
	...
	public Map<String, Object> updateCategory(HttpServletRequest request,
		...
		String icon = request.getParameter(Field.ICON.getKeyName());
		if (StringUtils.isEmpty(icon)) {
			model.fail("更新失败，icon不能为空！");
			return model.getDatas();
		}
		try {
			if (this.categoryService.updateCategory(super.getUserName(request),
					id, name, icon)) {
				model.success();
			} else {
				model.fail("更新失败！");
