...
public abstract class GenericDAO<T> implements IGenericDAO<T> {
	...
	@Override
	public List<T> getObjects(Long userid, List<Field> fields,
			Condition condition, OrderBy orderBy, Long offset, Long pageSize) {
		if (userid == null) {
			userid = DEFAULT_USERID;
		}
		StringBuilder sql = new StringBuilder();
		List<Object> params = new ArrayList<Object>();
		sql.append("select ").append(SqlBuilderHelper.getSelectSqlStr(fields))
				.append(" from ").append(getTableName(userid)).append(" ");
		if (condition != null) {
	...
	public int delete(Long userid, Condition condition) {
		if (condition == null) {
			throw new IllegalArgumentException(
					"delete operation without condition is not allowed. - "
							+ condition);
		}
		if (userid == null) {
			userid = DEFAULT_USERID;
		...
		StringBuilder sql = new StringBuilder();
		List<Object> params = new ArrayList<Object>();
		sql.append("delete from ").append(getTableName(userid))
				.append(" where ").append(condition.toSqlClause(params));
